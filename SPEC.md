...
21. **Referenced migration dropped the wrapper package (2026-09-26)** — Moving from `nix/outputs.nix` to `mkConsumerFlake` passed no `extraPackages`, so the flake exported only `packages.setting` and consumers running `lefthook-remote.yml` got exit 127; the devShell also lacked `is-markdown-agentic`, so unit tests could not find it. Fixed by restoring `default` and `is-markdown-agentic` through `extraPackages`, adding a `package` build check and a `flake.bats` test, and prepending both packages to every devShell. The pin bump also dropped the `checksFor` override from item 13, since `mkConsumerFlake` no longer accepts `lib`.
