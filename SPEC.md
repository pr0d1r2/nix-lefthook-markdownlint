...
18. **Guardrail fidelity remained incomplete after partial fragment alignment (2026-08-18)** — The consumer flake and its checks still omitted the `actions` fragment even though the dev shell and confirm app requested it, so CI regenerated `lefthook.yml` without the action-lint section. Fixed by declaring `actions` consistently in the consumer and check fragment lists.
19. **Nix formatting failure after guardrail configuration changes (2026-08-18)** — `guardrails / check` failed because `flake.nix` was not formatted according to the repository's nixfmt check. Fixed by applying the required nixfmt layout to the `checksFor` function binding.
20. **Project outputs were omitted from the consumer flake (2026-09-21)** — `flake.nix` returned only the generated consumer outputs, leaving the project’s markdownlint wrapper and agentic-file classifier unavailable in CI. Fixed by merging the outputs defined in `nix/outputs.nix` into the final flake outputs.
